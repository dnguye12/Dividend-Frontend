...
const FinancialsIncome = ({ ticker, stockQuote }) => {
    ...
    if ((loadingAnnual && showAnnual) || (loadingQuarter && !showAnnual)) {
        return (
            <div className="skeleton-bg w-full h-full min-h-screen"></div>
        )
    }
    ...
    if (!annualData || !quarterData) {
        return (
            <div>
                <p className="text-center text-text text-lg font-semibold mt-5">Income statement data is currently not available for {ticker}.</p>
            </div>
        )
    }
