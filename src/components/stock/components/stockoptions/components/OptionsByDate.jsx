...
const OptionsByDate = ({ ticker, optionsDates, stockOptions }) => {
    ...
    return (
        <div className="hidden md:block stock-options bg-base-950 border border-border rounded p-4 mt-4">
            <h2 className="text-text text-lg font-bold mb-3">{ticker} Options Activity by expiring date</h2>

            <div className="mb-3 flex justify-between">
                <div>
                    <div className="dropdown dropdown-bottom">
                        <div tabIndex={0} role="button" className="my-btn m-1">{moment(currentDate).format("MMM Do, YYYY")}</div>
                        <ul tabIndex={0} className="dropdown-content bg-base-800 rounded-box z-[10] w-52 shadow max-h-64 overflow-y-auto">
                            {
                                optionsDates.map((date, idx) => (
                                    <li key={idx} className="w-full hover:bg-neutral-700 px-3 py-2 transition duration-300 z-[10]">
                                        <p className={`block w-full cursor-pointer ${currentDate === date && 'font-semibold text-text'}`} onClick={() => {
                                            setCurrentDate(date)
                                            setDateIndex(idx)
                    ...
                    <div className="dropdown dropdown-bottom">
                        ...
                        <ul tabIndex={0} className="dropdown-content bg-base-800 rounded-box z-[10] w-52 shadow max-h-64 overflow-y-auto">
                            <li className="w-full hover:bg-neutral-700 px-3 py-2 transition duration-300 z-[10]"><p className={`block w-full cursor-pointer ${!currentStrike && 'font-semibold text-text'}`} onClick={() => {
                            ...
                            }}>All Strike Prices</p></li>
                            {
                                stockOptions[dateIndex].strikes.map((strike, idx) => (
                                    <li key={idx} className="w-full hover:bg-neutral-700 px-3 py-2 transition duration-300 z-[1]">
                                        <p className={`block w-full cursor-pointer ${strike === currentStrike && 'font-semibold text-text'}`} onClick={() => {
                ...
                <div>
                    <p className="text-text bg-blue-500 bg-opacity-20 text-base font-semibold my-btn border-none rounded-none hover:bg-blue-500 hover:bg-opacity-20 cursor-default">In the money</p>
                </div>
            </div>
                            ...
                            <tbody>
                                {
                                    currentStrike === null
                                        ...
                                        stockOptions[dateIndex].options[0].calls.map((call, idx) => (
                                            <tr className={`${call.inTheMoney && 'bg-blue-500 bg-opacity-20 text-text'}`} key={idx}>
