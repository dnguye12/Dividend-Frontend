...
const StockStatistics = ({ ticker }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                return (
                    <div className="bg-base-950 border border-border rounded p-4 border-spacing-10">
                        <p className="text-center text-text text-lg font-semibold">Statistics info is currently not available for {ticker}.</p>
                    </div>
                )
            }
        }
    ...
    if (!stockSummary) {
        return (
            <div className="w-full h-full skeleton border border-border bg-base-950 rounded"></div>
        )
    }
